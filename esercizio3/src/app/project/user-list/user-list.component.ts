...
import { Gender, Role, User } from 'src/app/models/models';
...
export class UserListComponent implements OnInit {
  ...
  deleteItem(submitted: number) {
    //const x=this.users.findIndex( (user) => submitted === user.id);
    //this.users.splice(x,1);
    this.users = this.users.filter((user) => user.id !== submitted);
  }
}
